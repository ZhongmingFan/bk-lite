    ...
    async def collect(self) -> str:
        ...
        for row in statuses:
            ...
            for peer in row.get("merakiVpnPeers") or []:
                if not isinstance(peer, dict):
                    continue
                peer_id = str(peer.get("networkId") or "")
        ...
        for row in stats:
            if not isinstance(row, dict):
                continue
            network_id = str(row.get("networkId") or "").strip()
            resource_id = serial_by_network.get(network_id) or network_id
            if not resource_id:
                continue
            current = metric_dict.setdefault((resource_id, "meraki_appliance"), {})
            for peer in row.get("merakiVpnPeers") or []:
