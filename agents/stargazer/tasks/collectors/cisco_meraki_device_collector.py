"""Cisco Meraki Dashboard API v1 device collector."""
...
class CiscoMerakiDeviceCollector(BaseCollector):
    """Collect device inventory, availabilities, and uplink loss/latency."""
    ...
    async def collect(self) -> str:
        ...
        uplink_timespan = _clamp_int(params["timespan"], 1, 300, 300)
        logger.info(
        ...
        async with MerakiDashboardClient(
            ...
            devices = await _optional_list(client, f"/organizations/{organization_id}/devices", {"perPage": 1000})
            availabilities = await _optional_list(
                client,
                f"/organizations/{organization_id}/devices/availabilities",
                {"perPage": 1000},
            )
            uplinks = await _optional_list(
                client,
                f"/organizations/{organization_id}/devices/uplinksLossAndLatency",
                {"timespan": uplink_timespan},
            )
        availability_by_serial = {str(item.get("serial") or ""): item for item in availabilities if item.get("serial")}
        online_count = sum(1 for item in availabilities if str(item.get("status") or "").lower() == "online")
        metric_dict = {
            (organization_id, "meraki_device_inventory"): {
                "meraki_device_connect_status": _gauge(1),
                "meraki_device_inventory_count": _gauge(len(devices)),
                "meraki_device_online_count": _gauge(online_count),
            }
        }
        for device in devices:
            ...
            if not serial:
                continue
            availability = availability_by_serial.get(serial, {})
            status_value = _status_code(availability.get("status"), AVAILABILITY_STATUS, 3)
            ...
            metric_dict[(serial, "meraki_device")] = {"meraki_device_availability_status": _dim_gauge(dims, status_value)}
        for row in uplinks:
            ...
            if latest is None:
                continue
            latency = _as_float(latest.get("latencyMs"))
            loss = _as_float(latest.get("lossPercent"))
            current = metric_dict.setdefault((serial, "meraki_device"), {})
            uplink_dims = [
                ("uplink", str(row.get("uplink") or "")),
                ("ip", str(row.get("ip") or "")),
            ]
            if latency is not None:
                current.setdefault("meraki_device_uplink_latency_ms", {}).update(_dim_gauge(uplink_dims, latency))
            if loss is not None:
                current.setdefault("meraki_device_uplink_loss_percent", {}).update(_dim_gauge(uplink_dims, loss))
        output = "\n".join(convert_to_prometheus(metric_dict)) + "\n"
        logger.info(
            "event=meraki_collect_success monitor_type=%s device_count=%s",
            MONITOR_TYPE,
            len(devices),
        )
        return output
