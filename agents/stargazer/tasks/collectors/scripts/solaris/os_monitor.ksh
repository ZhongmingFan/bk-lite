...
DISK_JSON=$(
  {
    ...
    END {
      ...
      for (i=1; i<=nm; i++) {
        ...
        gsub(/\\/, "\\\\", ft)
        gsub(/"/, "\\\"", ft)
        printf "{\"mount\":\"%s\",\"path\":\"%s\",\"fstype\":\"%s\",\"total_bytes\":%.0f,\"used_bytes\":%.0f,\"free_bytes\":%.0f,\"used_percent\":%.2f,\"inodes_used_percent\":%.2f,\"iused\":%.0f,\"ifree\":%.0f}", mm, mm, ft, tot[m]+0, ukb[m]+0, fkb[m]+0, pc[m]+0, ip_m[m]+0, iu_m[m]+0, ifr_m[m]+0
      }
      printf "]"
    }
  '
    ...
    END {
      ...
      if (src>0) {
        for (i=1; i<=n[src]; i++) {
          d=order[src, i]
          if (i>1) printf ","
          printf "{\"device\":\"%s\",\"read_bytes\":%.0f,\"write_bytes\":%.0f,\"tm_act\":%.2f}", d, rkb[src, d]*1024, wkb[src, d]*1024, tma[src, d]+0
        }
      }
      printf "]"
    }
