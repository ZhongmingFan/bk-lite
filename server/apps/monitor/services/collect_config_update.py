...
def stamp_applied(config, *, plugin_fp: str, rendered_content: str, hand_edited: bool = False):
...
class CollectConfigUpdateService:
    ...
    def annotate_instance_plugins(results: list[dict]) -> None:
        ...
        for item in results:
            ...
            for plugin_info in item.get("plugins") or []:
                ...
                bucket = stale_by_instance_plugin.get((instance_id, plugin_id))
    ...
    @staticmethod
    def _rerender_one(node_mgmt: NodeMgmt, config_obj: CollectConfig, plugin_fp: str) -> None:
        ...
        if is_config_hand_edited(config_obj, original):
            raise BaseAppException("采集配置已手改，禁止一键覆盖")
        ...
        config_obj.save(update_fields=["applied_content_sha256", "applied_rendered_sha256", "content_hand_edited", "updated_at"])

    @staticmethod
    def update_instances(instance_ids, plugin_id, actor_context) -> dict:
        raw_ids = [str(item) for item in (instance_ids or []) if item not in (None, "")]
        if not raw_ids:
            raise BaseAppException("instance_ids 不能为空")
        if len(raw_ids) > _MAX_UPDATE_INSTANCES:
        ...
        if plugin_id in (None, ""):
            ...
            plugins = [plugin]
        if not plugins:
            return {"updated": [], "skipped": [{"instance_id": item, "reason": "not_stale"} for item in raw_ids], "failed": []}

        merged = {"updated": [], "skipped": [], "failed": []}
        for plugin in plugins:
            result = CollectConfigUpdateService._update_instances_for_plugin(raw_ids, plugin, actor_context)
            merged["updated"].extend(result["updated"])
            merged["skipped"].extend(result["skipped"])
            merged["failed"].extend(result["failed"])
        merged["updated"] = list(dict.fromkeys(merged["updated"]))
        return merged

    @staticmethod
    def _update_instances_for_plugin(raw_ids, plugin: MonitorPlugin, actor_context) -> dict:
        ...
        config_instance_ids = {config_obj.monitor_instance_id for config_obj in configs}
        if config_instance_ids - authorized_ids:
            raise UnauthorizedException("无权限更新指定监控实例")
        ...
        for instance_id in raw_ids:
            ...
            for config_obj in instance_configs:
                if not is_config_stale(config_obj, plugin):
                    continue
                if is_config_hand_edited(config_obj):
                    instance_edited = True
                    continue
                try:
                    CollectConfigUpdateService._rerender_one(node_mgmt, config_obj, plugin_fp)
            ...
            elif instance_edited and not instance_failed:
                skipped.append({"instance_id": instance_id, "reason": "hand_edited"})
            elif not instance_failed:
                skipped.append({"instance_id": instance_id, "reason": "not_stale"})
    ...
    @staticmethod
    def mark_hand_edited(config_obj: CollectConfig, rendered_content: str, acknowledge_plugin: bool = True):
        plugin_fp = plugin_content_fingerprint(config_obj.monitor_plugin)
        applied_rendered = (config_obj.applied_rendered_sha256 or "").strip()
        content_changed = (not applied_rendered) or sha256_text(rendered_content) != applied_rendered
        if acknowledge_plugin and plugin_fp:
            config_obj.applied_content_sha256 = plugin_fp
        if content_changed:
            config_obj.content_hand_edited = True
        config_obj.save(update_fields=["applied_content_sha256", "content_hand_edited", "updated_at"])
