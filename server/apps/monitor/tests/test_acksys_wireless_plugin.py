...
BRAND_DIR = PLUGINS / "snmp" / "wireless_acksys"
...
PLUGIN_NAME = "Wireless ACKSYS SNMP"
...
def test_brand_match_and_icon_present_in_common_once():
    ...
    acks_lines = [line for line in text.splitlines() if "mm-acksys_acksys" in line]
    assert len(acks_lines) == 1
    assert r"acksys|airlink|waveos" in acks_lines[0].lower()
    assert "wireless|wifi|cellular|mesh|router|client|access" not in acks_lines[0].lower()
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-acksys_acksys.svg").exists()
...
def test_local_mock_instances_include_two_acksys_wireless_devices():
    ...
    assert text.count("instance_id: acksys_wireless_") == 2
...
def test_mock_metrics_registers_acksys_wireless_object():
    ...
    assert (
        ...
        '"config_type": "acksys", "instance_type": "wireless"}'
    ) in text
    assert '"acksys_wireless": "Wireless"' in text


@pytest.mark.unit
def test_no_duplicate_acksys_mapping_keys():
    wireless_text = (
        WEB_ROOT / "src" / "app" / "monitor" / "hooks" / "integration"
        / "objects" / "networkDevice" / "wireless.tsx"
    ...
    assert common_text.count("mm-acksys_acksys") == 1
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-acksys_acksys.svg",
    ]
