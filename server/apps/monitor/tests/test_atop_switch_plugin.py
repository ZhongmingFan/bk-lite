"""Contract tests for the ATOP Switch SNMP plugin.

ATOP exposes an enterprise root and official SNMP-based management tooling, but
public facts do not prove stable private health leaf OIDs. The plugin therefore
uses the shared SNMP floor and keeps vendor metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_atop"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "atop"
COLLECT_TYPE = "snmp_atop"
CONFIG_TYPE = "atop"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch ATOP SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.3755"
...
@pytest.mark.unit
def test_atop_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "ATOP" in text
    assert "mm-atop_atop" in text
    assert "atop" in text
    assert "nimbl" in text
    assert "atop|switch" not in text
    assert "atop|router" not in text
    assert "atop|industrial" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-atop_atop.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-atop_atop.svg"
    ]
    offenders = {
