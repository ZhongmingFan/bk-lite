"""Contract tests for the Ericsson transmission SNMP plugin."""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_ericsson"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "ericsson"
COLLECT_TYPE = "snmp_ericsson"
CONFIG_TYPE = "ericsson"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission Ericsson SNMP"
OBJECT_NAME = "Transmission"
PEN_ROOT = "1.3.6.1.4.1.193"
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "mm-ericsson_ericsson" in text
    assert text.count("label: 'Ericsson'") == 1
    assert text.count("mm-ericsson_ericsson") == 1
    assert "fiber access" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-ericsson_ericsson.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-ericsson_ericsson.svg"
    ]
    offenders = {
