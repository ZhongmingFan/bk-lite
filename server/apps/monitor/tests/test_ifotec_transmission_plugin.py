"""Contract tests for the Ifotec transmission SNMP plugin.

Ifotec industrial fiber-optic transceiver and optical transmission devices are
handled as a conservative baseline child: metrics.json declares no vendor metric
deltas, while the shared Transmission SNMP floor supplies uptime and
IF-MIB/ifXTable 64-bit HC traffic.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_ifotec"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "ifotec"
COLLECT_TYPE = "snmp_ifotec"
CONFIG_TYPE = "ifotec"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission Ifotec SNMP"
OBJECT_NAME = "Transmission"
PEN_OID = "1.3.6.1.4.1.21362"
...
def test_no_private_health_metrics_are_collected(toml_text):
    ...
    assert PEN_OID not in toml_text
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "mm-ifotec_ifotec" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-ifotec_ifotec.svg").exists()


@pytest.mark.unit
def test_brand_regex_does_not_match_generic_transmission_terms():
    """Ensure the Ifotec brand regex does not gobble generic transmission terms
    (e.g. fiber, optical, transceiver, sdh) that could mis-identify other
    vendors' instances as Ifotec.
    """
    common = WEB_ROOT / "src" / "app" / "monitor" / "utils" / "common.tsx"
    text = common.read_text(encoding="utf-8")
    forbidden = ("fiber", "optical", "transceiver", "sdh", "pon", "olt", "ftth", "gpon")
    for term in forbidden:
        assert f"/{term}/i" not in text, f"generic term {term!r} found in BRANDS regex"


@pytest.mark.unit
def test_collecttype_uniqueness_in_transmission_object():
    """Verify there is exactly one collect_type entry for the Ifotec plugin in
    the transmission.tsx collectTypes map (defense against 沉淀 #11 duplicate
    keys).
    """
    transmission_tsx = (
        WEB_ROOT / "src" / "app" / "monitor" / "hooks" / "integration"
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-ifotec_ifotec.svg",
    ]
