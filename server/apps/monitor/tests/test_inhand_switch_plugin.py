"""Contract tests for the InHand Networks Switch SNMP plugin.

Public facts prove InHand industrial switches support SNMPv1/v2c/v3 and
standard MIB-II / IF-MIB capability. No stable public private health leaf OIDs
are available, so the plugin must stay on the shared SNMP floor.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_inhand"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "inhand"
COLLECT_TYPE = "snmp_inhand"
CONFIG_TYPE = "inhand"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch InHand Networks SNMP"
OBJECT_NAME = "Switch"
...
@pytest.mark.unit
def test_inhand_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "InHand Networks" in text
    assert "mm-inhand_inhand" in text
    assert "inhand" in text
    assert "inhand|switch" not in text
    assert "inhand|industrial" not in text
    assert "inhand|ethernet" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-inhand_inhand.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-inhand_inhand.svg"
    ]
    offenders = {
