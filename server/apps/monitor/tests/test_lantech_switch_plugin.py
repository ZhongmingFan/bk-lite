"""Contract tests for the Lantech switch SNMP plugin.

Lantech industrial managed Ethernet switches are represented by enterprise PEN
148305. Current production input confirms the public enterprise root,
industrial switch product surface and official SNMP/MIB capability statements,
but not stable shared CPU, memory, temperature, fan or power leaf OIDs. The
plugin therefore remains conservative: it uses the shared SNMP floor only,
keeps metrics.json as a zero-delta child, and does not promote private
switch-management objects into shared device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_lantech"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "lantech"
COLLECT_TYPE = "snmp_lantech"
CONFIG_TYPE = "lantech"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch Lantech SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.148305"
...
def test_brand_label_present_once_in_common():
    ...
    assert "label: 'Lantech'" in text
    assert "mm-lantech_lantech" in text
    assert text.count("label: 'Lantech'") == 1
    assert text.count("mm-lantech_lantech") == 1
    assert "lantech|switch" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-lantech_lantech.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-lantech_lantech.svg"
    ]
    offenders = {
