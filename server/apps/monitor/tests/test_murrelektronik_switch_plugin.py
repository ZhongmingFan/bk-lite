"""Contract tests for the Murrelektronik Switch SNMP plugin.

Murrelektronik publishes industrial managed switch SNMP support and an
enterprise root, but the current public facts do not prove stable shared
private health leaf OIDs. The plugin therefore uses the shared SNMP floor and
keeps vendor metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_murrelektronik"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "murrelektronik"
COLLECT_TYPE = "snmp_murrelektronik"
CONFIG_TYPE = "murrelektronik"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch Murrelektronik SNMP"
OBJECT_NAME = "Switch"
...
@pytest.mark.unit
def test_murrelektronik_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Murrelektronik" in text
    assert "mm-murrelektronik_murrelektronik" in text
    assert "murrelektronik" in text
    assert "murrelektronik|switch" not in text
    assert "murrelektronik|industrial" not in text
    assert "murrelektronik|ethernet" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (
        WEB_ROOT / "public" / "assets" / "icons" / "mm-murrelektronik_murrelektronik.svg"
    ).exists()
