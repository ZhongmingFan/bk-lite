"""Contract tests for the RubyTech switch SNMP plugin.

RubyTech managed Ethernet switches have a confirmed enterprise PEN 5205 and
public switch product subtrees, but the current state has not confirmed public
...
metric deltas, while the shared Switch SNMP floor supplies uptime,
...
BRAND_DIR = PLUGINS / "snmp" / "switch_rubytech"
...
PLUGIN_NAME = "Switch RubyTech SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.5205"
...
def test_brand_match_and_icon_present_once_in_common():
    ...
    assert "label: 'RubyTech'" in text
    assert "mm-rubytech_rubytech" in text
    assert text.count("label: 'RubyTech'") == 1
    assert text.count("mm-rubytech_rubytech") == 1
    assert "rubytech|switch" not in text.lower()
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-rubytech_rubytech.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-rubytech_rubytech.svg",
    ]
