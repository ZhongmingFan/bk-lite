"""Contract tests for the Weidmuller switch SNMP plugin.

Weidmuller industrial Ethernet switches are represented by enterprise PEN
38187. Current production input confirms the public enterprise root,
industrial Ethernet product surface and official support center, but not stable
shared CPU, memory, temperature, fan or power leaf OIDs. The plugin therefore
remains conservative: it uses the shared SNMP floor only, keeps metrics.json as
a zero-delta child, and does not promote private objects into shared
...
BRAND_DIR = PLUGINS / "snmp" / "switch_weidmuller"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "weidmuller"
COLLECT_TYPE = "snmp_weidmuller"
CONFIG_TYPE = "weidmuller"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch Weidmuller SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.38187"
...
def test_brand_label_present_once_in_common():
    ...
    assert "label: 'Weidmuller'" in text
    assert "mm-weidmuller_weidmuller" in text
    assert text.count("label: 'Weidmuller'") == 1
    assert text.count("mm-weidmuller_weidmuller") == 1
    assert "weidmuller|switch" not in text.lower()
    assert "weidmuller|managed" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-weidmuller_weidmuller.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-weidmuller_weidmuller.svg"
    ]
    offenders = {
