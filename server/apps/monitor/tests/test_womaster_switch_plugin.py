"""Contract tests for the WoMaster Switch SNMP plugin.

WoMaster publishes official switch MIB downloads, but the current public facts
do not prove stable shared private health leaf OIDs. The plugin therefore uses
the shared SNMP floor and keeps vendor metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_womaster"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "womaster"
COLLECT_TYPE = "snmp_womaster"
CONFIG_TYPE = "womaster"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch WoMaster SNMP"
OBJECT_NAME = "Switch"
...
@pytest.mark.unit
def test_womaster_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "WoMaster" in text
    assert "mm-womaster_womaster" in text
    assert "womaster" in text
    assert "womaster|switch" not in text
    assert "womaster|industrial" not in text
    assert "womaster|ethernet" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-womaster_womaster.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-womaster_womaster.svg"
    ]
    offenders = {
