"""Contract tests for the XikeStor switch SNMP plugin.

XikeStor managed Ethernet switches have a public enterprise PEN and a public
support/download surface, but the current state has not confirmed public
...
BRAND_DIR = PLUGINS / "snmp" / "switch_xikestor"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "xikestor"
COLLECT_TYPE = "snmp_xikestor"
CONFIG_TYPE = "xikestor"
INSTANCE_TYPE = "switch"
PLUGIN_NAME = "Switch XikeStor SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.242853"
...
def test_brand_match_and_icon_present_once_in_common():
    ...
    assert "label: 'XikeStor'" in text
    assert "mm-xikestor_xikestor" in text
    assert text.count("label: 'XikeStor'") == 1
    assert text.count("mm-xikestor_xikestor") == 1
    assert "xikestor|switch" not in text.lower()
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-xikestor_xikestor.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-xikestor_xikestor.svg",
    ]
